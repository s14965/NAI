...
def define_parameters():
    ...
    params['first_layer_size'] = 200   # neurony w pierwszej warstwie
    params['second_layer_size'] = 20   # neurony w drugiej warstwie
    params['third_layer_size'] = 50    # neurony w trzeciej warstwie
    params['epoch'] = 250          
...
class Player(object):
    ...
    def do_move(self, move, x, y, game, food, agent):
        ...
        if np.array_equal(move, [1, 0, 0]):
            move_array = self.x_change, self.y_change
        elif np.array_equal(move, [0, 1, 0]) and self.y_change == 0:  # prawo w poziomie
            move_array = [0, self.x_change]
        elif np.array_equal(move, [0, 1, 0]) and self.x_change == 0:  # prawo w pionie
            move_array = [-self.y_change, 0]
        elif np.array_equal(move, [0, 0, 1]) and self.y_change == 0:  # lewo w poziomie
            move_array = [0, -self.x_change]
        elif np.array_equal(move, [0, 0, 1]) and self.x_change == 0:  # lewo w pionie
            move_array = [self.y_change, 0]
...
if __name__ == '__main__':
    # Ustawienia do opcji wyswietlania/trenowania/predkosci
    pygame.font.init()
